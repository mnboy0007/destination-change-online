...
def allocate_item(group: pd.DataFrame, item_rules: Dict[str, PriorityRule]) -> pd.DataFrame:
    ...
    if int(out["F Wk3 Original"].sum()) != int(out["F Wk3 After Destination Change"].sum()):
        raise ValueError(f"Item {out['Item'].iloc[0]}: tổng firm không bảo toàn.")
    ...
    with pd.ExcelWriter(output, engine="openpyxl") as writer:
        ...
        logic = pd.DataFrame(
            [
                ...
                ["Allocation rule", "Sau priority, luôn chọn kho có SS% After thấp nhất"],
            ],
...
st.write(f"Số item: **{df['Item'].nunique()}** | Số kho phát hiện: **{len(warehouses)}**")

st.subheader("Chọn kho ưu tiên")
st.caption("Chọn kho, rồi nhập trực tiếp tỷ lệ % mong muốn cho từng kho.")
selected_whs = st.multiselect("Chọn một hoặc nhiều kho priority", options=warehouses, default=[])
...
if selected_whs:
    ...
    for wh in selected_whs:
        with st.expander(f"Kho {wh}", expanded=True):
            c1, c2 = st.columns(2)
            with c1:
                mode = st.selectbox(f"Mode cho kho {wh}", options=["SI", "SS"], key=f"mode_{wh}")
            with c2:
                if mode == "SI":
            ...
            priority_rules[str(wh)] = PriorityRule(whse=str(wh), mode=mode, value=normalize_pct(value))
...
if st.button("Run Destination Change", type="primary"):
    try:
        ...
        if int(detail_full["F Wk3 Original"].sum()) != int(detail_full["F Wk3 After Destination Change"].sum()):
            raise ValueError("Sai tổng Firm PO toàn file.")
