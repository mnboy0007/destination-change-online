...
def compute_priority_target_final(row: dict, rule: PriorityRule) -> int:
    ...
    if rule.mode == "SI":
        # cover % of current SI toward zero
...
def build_priority_rule_ui(warehouses: List[str]) -> Dict[str, PriorityRule]:
    ...
    st.caption("Bạn chọn theo kho, không chọn theo item. Mỗi kho priority có thể đặt rule SI hoặc SS và tỷ lệ % riêng.")
