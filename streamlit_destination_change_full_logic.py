...
def safe_ss_ratio(current_si: float, ss_target: float) -> float:
    """
    %SS definition confirmed by user:
...
def find_vendor_columns(df: pd.DataFrame) -> List[str]:
    return [c for c in df.columns if "vendor" in str(c).lower()]
...
def load_input(uploaded_file) -> pd.DataFrame:
    ...
    # Keep Item as text so alphanumeric items are processed correctly.
...
def compute_priority_target_final(row: dict, rule: PriorityRule) -> int:
    ...
    if rule.mode == "SI":
        ...
        return max(0, int(round(final_f)))
...
def build_rows(group: pd.DataFrame, item_rules: Dict[str, PriorityRule]) -> Tuple[List[dict], int]:
    ...
    for _, r in group.iterrows():
        row = {
            ...
            "orig_f": int(round(float(r["F Wk3"]))),
            "current_si": int(round(float(r["Current SI"]))),
    ...
    total_f = int(round(float(group["Firm PO Total"].iloc[0])))
    return rows, total_f
        ...
        rows.append(
            {
                ...
                "Min SI After": float(g["Current SI After"].min()),
                "Max SI After": float(g["Current SI After"].max()),
...
if selected_items:
    ...
    for item in selected_items:
        ...
        item_df = df[df["Item"].astype(str) == item].copy()
        whs = sorted(item_df["Whse"].dropna().astype(str).unique().tolist())
        ...
        for wh in selected_whs:
            ...
            item_rules[item][str(wh)] = PriorityRule(
                whse=str(wh),
                mode=mode,
